...
// lấy genreId từ URL (vd: genre.html?id=5)
const params = new URLSearchParams(window.location.search);
const userId = parseInt(params.get("userId"));
const genreId = parseInt(params.get("genre"));
const genreName = params.get("genreName").replaceAll("_", " ");

function goHome() {
    window.location.href = `home?userId=${userId}`; // về trang chủ
}
...
async function loadMoviesByGenre() {
    ...
    if (!genreId) {
        document.getElementById("page-title").innerText = "Không có thể loại hợp lệ!";
        return;
    }

    // Gọi API lấy phim theo thể loại
    const json = await fetchApi(`http://localhost:8080/user/get-all-movies-by-genreId?userId=${userId}&genreId=${genreId}`);
    ...
    document.getElementById("page-title").innerText = `Thể loại: ${genreName}`;

    const container = document.getElementById("movies");
    container.innerHTML = "<p>Đang tải phim...</p>";
